...
final class DefinitionParser
{
    ...
    private function parseFunctions(\ConstVector<ScannedFunction> $functions) : void
    {
        array_walk($functions->toArray(), $function ==> {
            ...
            try {
                ...
                $serviceName = $this->checkServiceName($function->getAttributes()->get('Provides'));
                if($serviceName === null) {
                    return;
                }
                ...
                if($return === null) {
                    throw new \UnexpectedValueException('You must specify a return value.');
                }

                $this->services->add(shape(
                    'name' => $serviceName,
    ...
    {
        try {
            $serviceName = $this->checkServiceName($method->getAttributes()->get('Provides'));
            if($serviceName === null) {
                return;
            }
    ...
    private function checkServiceName(?Vector<mixed> $serviceAttribute) : ?string
    {
    ...
    private function checkName(string $name) : void
    {
        if($this->names->contains($name)) {
            $otherService = $this->services
                ->filter($s ==> $s['name'] === $name)
                ->at(0)
                ;

            $otherFunction = Shapes::idx($otherService, 'function') ?
                Shapes::idx($otherService, 'function', '') :
                Shapes::idx($otherService, 'class', '') . '::' . Shapes::idx($otherService, 'method');
