...
final class DefinitionParser {
    ...
    array_walk(
      ...
      $function ==> {
        ...
        try {
          ...
          $commandName =
            $this->checkCommandName($attributes->get('Command'));
          if ($commandName === null) {
            return;
          }
          $middleware =
            $this->checkMiddleware($attributes->get('UseMiddleware'));
  ...
  private function checkCommandName(
    ?Vector<mixed> $commandAttribute,
  ): ?string {
  ...
  private function parseOptionName(
    ...
    if ($this->optionHasAlias($name)) {
      ...
      if (strlen($alias) > 1) {
        throw new \UnexpectedValueException(
          'The alias of an option must be one character long.',
        );
      }
    ...
    return
      $arguments->map(
        $a ==> {
          ...
          if (!is_string($a)) {
            throw new \UnexpectedValueException(
              'The definition of an argument must be a string.',
            );
          }
          ...
          if (strpos($a, '=') === false) {
            if ($required->at(0) === false) {
              throw new \UnexpectedValueException(
                'All arguments with default values must be at the end of the list.',
              );
            }
  ...
  private function checkParameters(
    ...
    if ($requiredParamNames->count() !== 3 ||
      ...
      throw new \UnexpectedValueException(
        'Command handlers must except exactly 3 parameters:'.
        PHP_EOL.
  ...
  private function checkReturnType(?ScannedTypehint $returnType): void {
    if ($returnType === null || $returnType->getTypeName() !== 'int') {
      throw new \UnexpectedValueException(
